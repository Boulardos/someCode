...
const provider = new HDWalletProvider(process.env.BOTprivK, 'https://mainnet.infura.io/v3/262187caeacd4fe8ad8e761bfb3665e2');
...
const nftAddr = "0x4a8C9D751EEAbc5521A68FB080DD7E72E46462aF";
const NFT = new web3.eth.Contract(nftAbi, nftAddr);
const Abi = new ethers.utils.Interface(routerabi);
const pcsAbi = new ethers.utils.Interface(routerabi);
let token = cakeAddress; 

const methodId = () => { // !!!!!! pas d'espace entre les parametres ex: swapExactTokensForETH(uint256,uint256,address[],address,uint256)
   const selector = web3.eth.abi.encodeFunctionSignature("finalize()");
 // "swapExactTokensForTokensSupportingFeeOnTransferTokens(uint,uint,address[],address,uint)");
    console.log('methodId', selector);
    // flipMintState()
}
methodId();
	...
	{  
        ...
        if (tx && tx.to) {
          if (ethers.utils.getAddress(tx.to) === 
              ethers.utils.getAddress(nftAddr)){
		    // console.log('tx pcs router pending');	
            const re1 = new RegExp("^0x59c74f29"); // flipMintState()	  
            const re2 = new RegExp("^0x7ff36ab5"); //  swapExactETHForTokens
            
            // if (re1.test(tx.data) || re2.test(tx.data))
			if (re1.test(tx.data))
			 {
				console.log(' flipmint tx pending'); 
                const decodedInput = nftAbi.parseTransaction({
                data: tx.data,
                value: tx.value,
              });
			...
			//  {
                ...
                let quantity = 2; // max 3
            	let amount = 250 * quantity; // amount BNB  1000 = 1 BNB
            	let BNamount = new BN(amount).mul(new BN(oneE15)); 
			
				await router.methods.mintStandardLands(2)
			     .send({from: myaccount, value: BNamount, gas: '2000000',
			     gasPrice: web3.utils.toWei("10", 'gwei')});
			// 	console.log('TX SENT____________________'); 
              // }
            }
          }
        }
